  ...
  render() {
    ...
    const itemHTML = items.map((item) => (
      <Col xs={6} sm={4} lg={3.5} style={{ paddingTop: 15, paddingBottom: 15 }} key={item.id}>
        <button type="button" onClick={(e) => this.displayMovie(item, e)} aria-label={item.title}><GridCardView item={item} id={item.videoAsset.id} title={item.videoAsset.title} details={item.videoAsset.description}/></button>
      </Col>
    ));
